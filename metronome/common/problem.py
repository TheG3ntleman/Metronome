...
class SyntheticProblemGenerator:
    ...
    def generate_feasible_specifications() -> TimeTableSpecifications:
        ...
        number_of_blocks = 10
        matrix = np.zeros((35, 20, 70)) # (Timeslots, Venues, Parties)  
        EclairField_object = EclairField(number_of_blocks, specifications, matrix)
        block_list = EclairField_object.generate_blocks()
        matrix = EclairField_object.generate_matrix(block_list[0], block_list[1], block_list[2])
        ...
        # How do i get the session type and course
        # also, how do i identify the repeatitions in the matrix using blocks
        # still need to do assignments table
    ...
    def generate_matrix(self, blocks, block_repeat, capacities):
        ...
        for j in range(number_of_blocks):  # for the block
            ...
            for _ in range(block_repeat[j]):
                ...
                capacity = capacities[j] # for every repeatition, the capacity remains the same
                venue_id, venue_type = self.find_the_venue(capacity) # for every repeatition, the venue also remains the same
                ...
                if venue_type == 4:
                    ...
                    for party in blocks[j]: # lists out the parties in the block

                        timeslot: int = -1

                        for i in range(number_of_timeslots - 1):
                            if self.matrix[i][venue_id][party] == 0 and self.matrix[i + 1][venue_id][party] == 0 and i%7 == (i+1)%7:  # this basically indicated the same day
                                timeslot = i
                                print("TimeSlot for lab: ", timeslot);
                                break

                        if timeslot == -1:
                            # There's no timeslot available for the party
                            break

                        self.matrix[timeslot][venue_id][party] = session_id
                        self.matrix[timeslot + 1][venue_id][party] = session_id
                    session_id +=1

                else:
                  for party in blocks[j]:
                          # Now we need to mark the venue and all the parties in the block with the corresponding timeslot
                          # timeslot, venue, party
                          # choose a random timeslot
                          timeslot: int = -1
                          # Timeslot and venue are already occupied, We can't change the venue, so we are changing the timeslot

                          for i in range(number_of_timeslots):
                              if self.matrix[i][venue_id][party] == 0:
                                  timeslot = i
                                  break
                          if timeslot == -1:
                              # There's no timeslot available for the party
...
if __name__ == "__main__":
    ...
    a = SyntheticProblemGenerator.generate_feasible_specifications() # TimeTableSpecifications object
    # (Timeslots, Venues, Parties)
    number_of_blocks = 10
    b = EclairField(number_of_blocks, a, np.zeros((35, 20, 70))) # Number of blocks = 10
    c = b.generate_blocks()
    d = b.generate_matrix(c[0], c[1], c[2])
    print(np.shape(d))
    cnt: int = 0
    for i in range(35):
      for j in range(20):
        for k in range(70):
          c: int = 0
          if d[i][j][k] != 0:
            print("Timeslot: ", i, "Venue: ", j,"Party: ", k, "Session: ", d[i][j][k])
            cnt += 1
    print()
    print("Count: ", cnt)
                      # working fine now
