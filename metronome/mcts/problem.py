...
class Problem:
  ...
  def __init__(self, number_of_sessions, options):
...
class MCTSTupleFrequencyProblem(Problem):
  ...
  def __init__(self, population, number_of_timeslots, number_of_venues, top_k = 5):
    ...
    for session_id in range(population[0].number_of_sessions):
      ...
      for timetable_id in range(len(population)):
        session_info = population[timetable_id].get_session_info(session_id)
    ...
    super().__init__(population[0].number_of_sessions, options)
