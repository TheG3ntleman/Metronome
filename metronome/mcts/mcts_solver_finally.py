...
class MCTS_solver:
    ...
    def back_propogate(self, violations, added_node, node):
        ...
        if node.depth == self.time_table_specifications.number_of_sessions - 1:
            ...
            while node is not None:
                node.visit_count += 1
                node.cummulative_violations += 500*violations*(self.time_table_specifications.number_of_sessions - added_node.depth - 1)
                if added_node.depth == node.depth:
                    node.children = []
                    # adding children
                    for i in range(self.k):
        ...
        while n_solutions < 1:
            ...
            if current_node.depth == self.time_table_specifications.number_of_sessions - 1:
                ...
                for i in range(self.time_table_specifications.number_of_sessions):
                    print(self.partial_timetable.partial_timetable[i]["timeslot_id"], self.partial_timetable.partial_timetable[i]["venue_id"])
                continue
            ...
            '''
            if current_node.depth == added_node.depth:
