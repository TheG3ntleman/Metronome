...
numeric soft_constraint_student_travel_time(TimeTableEntry *timetable, uint n_sessions, TimeTableSpecifications *specs) {
...
numeric soft_constraint_maximize_chunking(TimeTableEntry *timetable, uint n_sessions, TimeTableSpecifications *specs) {
...
numeric soft_constraint_room_utlisation(TimeTableEntry *timetable, uint n_sessions, TimeTableSpecifications *specs) {
  ...
  for (uint i = 0; i < specs->venue_table->size; i++) { // Corrected venue_table to venue_table->size
...
numeric soft_constraint_avoid_early_time(TimeTableEntry *timetable, uint n_sessions, TimeTableSpecifications *specs) {

  /* Avoid Early/Late Times Constraint: Express preferences to avoid scheduling
   * classes too early or too late in the day. */
  numeric violations = 0;
  uint number_of_days = specs->timeslot_table->day[specs->timeslot_table->size - 1];
  uint number_of_timeslot_per_day = specs->timeslot_table->size / number_of_days;
...
numeric soft_constraint_room_capacity_utilisation(TimeTableEntry *timetable, uint n_sessions, TimeTableSpecifications *specs) {
...
numeric soft_constraint_common_timeslot_empty(TimeTableEntry *timetable, uint n_sessions, TimeTableSpecifications *specs) {
...
numeric soft_constraint_minimize_backtoback_teacher_classes(TimeTableEntry *timetable, uint n_sessions, TimeTableSpecifications *specs) {
...
numeric soft_constraint_minimize_samecourse_sessions(TimeTableEntry *timetable, uint n_sessions, TimeTableSpecifications *specs) {
...
numeric soft_constraint_evenly_throughout_week(TimeTableEntry *timetable, uint n_sessions, TimeTableSpecifications *specs) {
  ...
  for (uint i = 0; i < specs->party_table->size; i++) {
    ...
    for (uint j = 0; j < number_of_session; j++) {
      ...
      uint timeslot_id = timetable[session_id_array[j]].timeslot;
      uint venue_id = timetable[session_id_array[j]].venue;
      uint day = specs->timeslot_table->day[timeslot_id]; // Corrected day access
      day_array[day]++;
...
snumeric get_optimality(TimeTableEntry *timetable,
    ...
    fitness += soft_constraint_student_travel_time(timetable, specifications->session_table->size, specifications);

    fitness += soft_constraint_avoid_early_time(timetable, specifications->session_table->size, specifications);

    fitness += soft_constraint_common_timeslot_empty(timetable, specifications->session_table->size, specifications);

    fitness += soft_constraint_maximize_chunking(timetable, specifications->session_table->size, specifications);

    fitness += soft_constraint_minimize_backtoback_teacher_classes(timetable, specifications->session_table->size, specifications);

    fitness += soft_constraint_minimize_samecourse_sessions(timetable, specifications->session_table->size, specifications);

    fitness += soft_constraint_room_capacity_utilisation(timetable, specifications->session_table->size, specifications);

    fitness += soft_constraint_room_utlisation(timetable, specifications->session_table->size, specifications);

    fitness += soft_constraint_evenly_throughout_week(timetable, specifications->session_table->size, specifications);
