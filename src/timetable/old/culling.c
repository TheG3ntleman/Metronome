...
unsigned int assignToCluster(Population* population, unsigned int schedule_index, Cluster* clusters, unsigned int* num_clusters, unsigned int min_schedules_per_cluster) {
    ...
    for (unsigned int i = 0; i < *num_clusters; ++i) {
        numeric similarity = calculateTimeTableSimilarity(population, schedule_index, clusters[i].schedules[0]); // Assuming comparing with the first schedule in the cluster, why first one? cause finding centroid is a pain in the ass and the schedules in a cluster are similar
...
void cullPopulation(Population* population, numeric* soft_fitness, numeric* hard_fitness, unsigned int selection_size, unsigned int* selection_arr, unsigned int min_schedules_per_cluster) {
    unsigned int total_schedules = population->size;  // Assuming population size is available
	...
	// No of clusters and schedules must be returned
