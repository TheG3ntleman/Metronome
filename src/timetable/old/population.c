...
numeric calculateTimeTableSimilarity(Population *population, uint i1, uint i2) {
	...
	for (uint i = 0; i < population->n_sessions; i++) {
		if (population->tt_indices[i1 * population->n_sessions + i] == population->tt_indices[i2 * population->n_sessions + i])
			similarity++;
	}
...
void getTimeTableTuple(Population *population, uint tt_id, uint session_id, uint *venue_id, uint *timeslot_id) {
	...
	uint tuple_index = population->tt_indices[population->n_sessions * tt_id + session_id];
