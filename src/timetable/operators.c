...
void operatorInitialize(Population *population, TimeTableSpecifications *ttSpecs, GeneticSpecifications *gaSpecs) {
	...
	for (uint t = 0; t < population->n_timetables; t++) {
		// Iterating through all sessions
			...
			uint venue_id = ttSpecs->venues->venue_id[randint(0, ttSpecs->venues->size - 1)];
...
void operatorCrossover(Population *new_population, Population *current_population, uint *indices_of_selected_organisms, TimeTableSpecifications *ttSpecs, GeneticSpecifications *gaSpecs) {
	...
	for (uint t_i = 0; t_i < current_population->n_timetables; t_i++) {
		...
		uint s_id_1 = indices_of_selected_organisms[rand() % gaSpecs->selection_size];
		uint s_id_2;
		...
		// Now performaing a tuple-wise random crossover between these two timetables
